...
%CM1 Received Signal
CM1_channel_output = conv(allPulses,h_channel); 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Iteration To Calculate BER-SNR

BER_SNR_Conventional = zeros(1,11);
BER_SNR_Bybrid = zeros(1,11);



title('BEP versus SNR for the conventional receiver and the hybrid receiver')
xlabel('SNR(dB)')
...
for AWGNSNR = 0:2:20
    ...
    %Integration to form estimated data sequence per pulse
    %Integration Time of Hybrid Filter: Q_Interval
    % From Td-Tc to Td+QTc
    Q_Interval;
    ...
    for i = 1:numberOfBits
            ...
            Start_Point = ((Guard_Interval+Ts)*(i-1) + Tf*(Nf-1) + TH_Sequence(1,Nf)+Td)*ChipPoints;
                for j = 1:Q_Interval     
                    if (Start_Point+j < SizeOfSignals+1)
    ...
    BER_SNR_Conventional(1,Position) = BER_Conventional;
    BER_SNR_Hybrid(1,Position) = BER_Hybrid;
    
    
    BER_Q_Conventional(1,Q) = BER_Conventional;
    BER_Q_Hybrid(1,Q) = BER_Hybrid;
...
if(Q<=20)
    semilogy(SNR_X_Axis,BER_SNR_Conventional,'-o',SNR_X_Axis,BER_SNR_Hybrid,'--*');
    hold on
end
end
end
legend('BER_SNR_Conventional Q=1','SNR_X_Axis,BER_SNR_Hybrid Q=1', 'BER_SNR_Conventional Q=10','SNR_X_Axis,BER_SNR_Hybrid Q=10', 'BER_SNR_Conventional Q=20','SNR_X_Axis,BER_SNR_Hybrid Q=20')
hold off
